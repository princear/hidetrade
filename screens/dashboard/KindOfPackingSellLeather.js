...
const KindOfPackingSellLeather = (props) => {
  const [kindOfPacking, setKindOfPacking] = useState(undefined);
  ...
  const renderKindOfPacking = ({ item, index }) => {
    ...
    return (
      ...
      >
        <Image
          ...
          source={{
            uri:
              "https://refuel.site/projects/hidetrade/APIs/ViewAllPackagingArrangeList/" +
              image_name,
          }}
        />
      </TouchableOpacity>
  ...
  useEffect(() => {
    if(dataLoad==false){
      setApiLoader(true);
      let webApiUrl = `https://refuel.site/projects/hidetrade/APIs/ViewAllPackagingArrangeList/ViewAllPackagingArrangeList.php`;
      axios.get(webApiUrl).then((res) => {
        console.log("response in new screen=" + JSON.stringify(res.data));
        setKindOfPacking(res.data);
        setApiLoader(false);
        setDataloaded(true);
      });
    }
    ...
    <View style={{ flex: 1, backgroundColor: "white" }}>
      ...
      <ScrollView>
        <View style={{ marginHorizontal: 10 }}>
          ...
          >
            What kind of packing can you arrange?
          </Text>
          {kindOfPacking != undefined ? (
            <View>
              <FlatList
                numColumns={3}
                data={kindOfPacking.Output}
                ...
                //   </TouchableOpacity>
                // )}
                renderItem={renderKindOfPacking}
                scrollEnabled={false}
              />
            </View>
          ) : null}
        ...
        <View style={{ flexDirection: "row", marginBottom: 15 }}>
          ...
          >
            <TouchableOpacity
              ...
              // onPress={() =>
              //   props.navigation.navigate("KindOfShipmentSellLeather",{productName:productName, category:category, subCategory:subCategory, size:size,leatherCondition:leatherCondition, tanningLeather:tanningLeather,substanceThickness:substanceThickness, fromValue:fromValue, toValue:toValue, destination:destination, trim:trim, flay:flay, rawDefects:rawDefects, hairLeather:hairLeather, color:color, certificate:certificate, kindOfPacking:selected})
              // }
              onPress={() => {
                if (leatherCondition == "Raw") {
                  props.navigation.navigate("Shipment Type", {
...
export default KindOfPackingSellLeather;
